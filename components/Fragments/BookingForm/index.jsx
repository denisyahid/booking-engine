...
export default function BookingForm() {
    ...
    const handleCheckInChange = (e) => {
        ...
        setBookingData(prev => ({ ...prev, checkIn: newCheckIn }));
    };
        ...
        <div className='min-h-screen bg-gray-100 flex justify-center py-6 px-3'>
            <div className='w-full max-w-5xl bg-white shadow-lg overflow-hidden'>
                ...
                {checkingAvailability && (
                    <div className='mx-6 mt-4 p-3 bg-blue-50 border border-blue-200 text-blue-600 rounded-lg text-sm'>
                ...
                <form onSubmit={handleSubmit} className='grid md:grid-cols-3 gap-6 px-6'>
                    ...
                    <div className='md:col-span-2 space-y-6'>
                        ...
                        <div className='border p-5 shadow-sm'>
                            ...
                            <div className='flex justify-between text-sm text-gray-700 mt-2'>
                                <p>Check-In: <span className='font-medium'>{getDayName(bookingData.checkIn)} {bookingData.checkIn}</span></p>
                                <p>Check-Out: <span className='font-medium'>{getDayName(bookingData.checkOut)} {bookingData.checkOut}</span></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='space-y-6'>
                        ...
                        <div className='border shadow-sm'>
                            ...
                            <div className='p-4 space-y-2'>
                                ...
                                <p className='text-sm text-gray-600'>
                                    ...
                                    <br />
                                    {pricing.nights} Night{pricing.nights !== 1 ? 's' : ''} • {bookingData.roomQuantity} Room{bookingData.roomQuantity !== 1 ? 's' : ''} • {rates.room?.capacity || '-'} Guest
                                </p>
                                <p className='text-right font-bold text-primary'>{formatRupiah(pricing.baseRate)}</p>
                            </div>
                        </div>
                        ...
                        {/* Price Breakdown */}
                        <div className='border p-4 shadow-sm space-y-3'>
                            <h2 className='font-semibold text-lg'>Price Details</h2>
                            <div className='space-y-2 text-sm'>
                                <div className='flex justify-between'>
                                    <span>{formatRupiah(pricing.baseRate)} × {bookingData.roomQuantity} room{bookingData.roomQuantity !== 1 ? 's' : ''} × {pricing.nights} night{pricing.nights !== 1 ? 's' : ''}</span>
                                ...
                                <div className='flex justify-between'>
                                    <span>Tax (11%)</span>
                                    <span>{formatRupiah(pricing.tax)}</span>
                                </div>
                                <div className='flex justify-between'>
                                    <span>Service Charge (5%)</span>
                                    <span>{formatRupiah(pricing.serviceCharge)}</span>
                                </div>
                                <hr className='my-2' />
                                <div className='flex justify-between text-lg font-bold'>
                                    <span>Total</span>
                                    <span className='text-primary'>{formatRupiah(pricing.total)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className='border p-4 shadow-sm space-y-4'>
                            ...
                            <button
                                type='submit'
                                disabled={submitting || paymentProcessing || checkingAvailability}
                                ...
                                {submitting ? 'Processing...' : paymentProcessing ? 'Opening Payment...' : checkingAvailability ? 'Checking...' : 'Continue To Payment'}
                            </button>
                        </div>
                    </div>
                </form>
