...
import { 
  FaSwimmingPool, FaWifi, FaChild, FaCouch, 
  FaCalendarAlt, FaFire, FaMountain, FaCoffee, FaBath 
} from 'react-icons/fa';
...
export default function Facilities() {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:8000/api/${slug}/facilities`)
        .then((res) => {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:8000/api/${slug}/facilities/categories`)
        .then((res) => {
    ...
    return (
        <div className='max-w-6xl mx-auto bg-white p-6 shadow rounded-lg my-6'>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
                {facilities
                    ...
                    .map((item, idx) => (
                        <div 
                          key={idx} 
                          className='flex items-center gap-3 text-gray-700 border rounded-lg p-4 shadow-sm'
                        >
            ...
            {/* Tombol Lihat Semua */}
            <button 
              className='mt-4 text-primary font-semibold text-sm md:text-base' 
